...
```{r}
demo <- rename(demo, Child=Child.ID)

lu <- rename(lu, Child=SUBJ, Visit=VISIT)

words <- rename(words, Child=SUBJ, Visit=VISIT)
...
words$Visit <- stringr::str_extract(words$Visit, "\\-*\\d+\\.*\\d*")
lu$Visit <- as.numeric(lu$Visit)
words$Visit <- as.numeric(lu$Visit)
...
New_merge <- rename(New_merge, Visits = Visits.x, Ethnicity = Ethnicity.x, Diagnosis = Diagnosis.x, Gender = Gender.x, Age = Age.x, MOT_MLU = MOT_MLU.x, MOT_LUstd = MOT_LUstd.x, CHI_MLU = CHI_MLU.x, CHI_LUstd = CHI_LUstd.x,types_MOT = types_MOT.x, types_CHI = types_CHI.x, tokens_MOT = tokens_MOT.x, tokens_CHI = tokens_CHI.x)


#creating a new dataset with only the ADOS from visit 1
Only_visit_1 <- subset(New_df, Visits == 1)
```
